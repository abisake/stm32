...
static void task_button(void *pvParams)
{
	uart_puts("Button Task ready, Press Blue button\r\n");
	...
	while(1)
	{
		...
		button_press_count++;
		uart_print_u32("Button Press count=", button_press_count);

		/* Ignore the signals for 200 ms after a press
		 * This is called Software bounce
		 * Any bounces that fired the ISR again are drained here.
		 * */
...
static void task_hearbeat(void *pvParams)
{
	 ...
	 while(1)
	 {
		 ...
		 uart_print_u32("HB tickCount=",xTaskGetTickCount());

		 // printing heap size
		 uart_print_u32("HB free_heap=", xPortGetFreeHeapSize());
		 uart_print_u32("HB btn press count=", button_press_count);
	 }
}
...
int main(void)
{
  ...
  if(!xUartMutex || !xRxQueue || !xButtonSemaphore)
  {
  ...
  BaseType_t r4 = xTaskCreate(task_button, "BTN", 256, NULL, 3, NULL);
